...
void Trie::insert(string word) {
    ...
    for (int i = 0; i < word.length(); i++) {
        ...
        if(pt->children[i]==NULL){
            ...
            pt->children[wordIndex]=currNode;
        }
        pt=pt->children[wordIndex];
    }
...
int Trie::search(string word) {
    ...
    for (int i = 0; i < word.length(); i++){
        ...
        if(pt->children[tempIndex]==NULL){
            return -1;
        }
        pt=pt->children[tempIndex];
        wordIndex=tempIndex;

    }
    if(pt->isEndOfWord==true){
        return wordIndex;
    }else{
        return -1;
    }
